...
class Balloon:
    ...
    def is_clicked(self, pos): dx = pos[0]-self.x; dy = pos[1]-self.y; return dx*dx+dy*dy<=self.radius*self.radius
...
class Game:
    def __init__(self):
        ...
        self.game_w = min(1000, WIDTH); self.game_h = min(800, HEIGHT)
        self.offset_x = (WIDTH-self.game_w)//2; self.offset_y = (HEIGHT-self.game_h)//2
        ...
        self.interval_up_moves = [0] * INTERVAL_COUNT
        self.interval_down_moves = [0] * INTERVAL_COUNT
        self.balloons = []
        ...
        for c in range(cycles):
            ...
            for i, count in enumerate(PATTERN):
                ...
                if phase_start >= GAME_DURATION: break
                interval = STEP_DURATION / count
                for j in range(count):
                    t = phase_start + j * interval
                    if t < GAME_DURATION: self.red_schedule.append(t)
        self.red_schedule.sort()
        self.next_red_idx = 0
        self.last_nonred = pygame.time.get_ticks()
        self.nonred_spawned = 0
    def get_speed(self, elapsed):
        cp = elapsed % CYCLE_DURATION; idx = min(int(cp // STEP_DURATION), PHASE_COUNT - 1)
        half = PHASE_COUNT // 2; r = idx/half if idx <= half else (PHASE_COUNT-1-idx)/half
        return 0.04 + (0.20 - 0.04) * max(0.0, min(r, 1.0))
    def save_data(self):
        od_dir = os.path.expanduser('~/OneDrive/Desktop/Mendi_vs_Octamon_Study/Balloon_performance')
        fn = f"{PARTICIPANT_ID}_balloon_performance.csv"
        os.makedirs(od_dir, exist_ok=True)
        od_path = os.path.join(od_dir, fn)
        # remove existing file to avoid permission conflicts
        if os.path.exists(od_path):
            try:
                os.remove(od_path)
            except Exception as e:
                print(f"Could not remove existing OneDrive file: {e}")
        try:
            with open(od_path, 'w', newline='') as f:
                w = csv.writer(f)
                w.writerow(['interval_start_s','spawned','hits','misses','avg_reaction_ms','line_up','line_down','hit_rate'])
                interval_s = STEP_DURATION // 1000
                for i in range(INTERVAL_COUNT):
                    start_s = (i+1) * interval_s
                    spawned = self.interval_spawned[i]
                    hits = self.interval_hits[i]
                    down = self.interval_down_moves[i]
                    misses = down
                    rts = self.interval_reactions[i]
    ...
    def run(self):
        ...
        while running:
            dt = self.clock.tick(FPS); now = pygame.time.get_ticks(); elapsed = now - self.start_time
            if elapsed >= GAME_DURATION: break
            spd = self.get_speed(elapsed)
            ...
            while self.next_red_idx < len(self.red_schedule) and elapsed >= self.red_schedule[self.next_red_idx]:
                ...
                self.balloons.append(Balloon(x, self.offset_y, RED, spd, elapsed))
                self.interval_spawned[idx] += 1
                self.next_red_idx += 1
            if now - self.last_nonred >= 600 and self.nonred_spawned < NONRED_BALLOONS_TOTAL:
                x = random.randint(self.offset_x+20, self.offset_x+self.game_w-20)
                self.balloons.append(Balloon(x, self.offset_y, random.choice(COLORS), spd, elapsed))
                self.nonred_spawned += 1; self.last_nonred = now
            for e in pygame.event.get():
                if e.type == pygame.QUIT or (e.type == pygame.KEYDOWN and e.key == pygame.K_ESCAPE): running = False
                if e.type == pygame.MOUSEBUTTONDOWN:
                    p = pygame.mouse.get_pos()
                    for b in self.balloons[:]:
                        if b.color == RED and b.is_clicked(p):
                            rt = elapsed - b.spawn_time
                            self.interval_reactions[idx].append(rt); self.interval_hits[idx] += 1
                            self.interval_up_moves[idx] += 1
                            self.line_y = max(self.offset_y+LINE_MARGIN, self.line_y-10)
                            self.crosshair_color = CROSSHAIR_GREEN; self.shot_timer = now
                            self.balloons.remove(b); break
            if now - self.shot_timer > 150: self.crosshair_color = CROSSHAIR_RED
            drag = 0
            for b in self.balloons[:]:
                b.speed = spd; b.update(dt)
                if b.color == RED and not b.touched_line and b.y+b.radius >= self.line_y:
                    b.touched_line = True; self.interval_down_moves[idx] += 1
                if b.color == RED and b.y+b.radius >= self.line_y: drag = max(drag, b.speed)
